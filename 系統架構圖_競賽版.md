...
| Agentic 要素 | 本系統落點 | 狀態 |
|---|---|---|
| **Harness Engineering** | 驗證與應變皆為結構化管線（validators → resolver → enrich → ingest；detect → EV → retrieve → reflect → generate），每個 LLM 呼叫都被程式框架包裹（schema 驗證、重試、防截斷） | ✅ 已實作 |
...
| 信任評分＋L0–L3 分級 | `agents/poi-verifier/src/enrichers/` |
