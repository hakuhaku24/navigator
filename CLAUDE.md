...
4. **部落格佐證混入不相關內容——現在就在畫面上**。曾有 90 則佐證中 11 則來自 YouTube，出現與景點完全無關的影片（擎天崗的佐證裡有勞斯萊斯開箱）。`youtube-search` 濾了業配，沒濾相關性。
   ⚠️ **2026-08-18 更正（`03b0726` 黑箱測試 F7）**：本檔原本寫「線上 `youtube_video_count > 0` 為 0/100，所以這個問題現在不會出現在畫面上，一旦重新啟用才會回來」——**這個結論是錯的**。實測 6 個景點的佐證清單都有汙染，而且**來源標記全是 `duckduckgo` 不是 youtube**：陽明書屋（「2025 in anime - Wikipedia」＋勞斯萊斯開箱）、中山樓（勞斯萊斯）、石門婚紗廣場（「#2025 YouTube」「#2025 TikTok」）、神秘海岸（澳洲旅行社評測）、潮境公園（南投埔里懶人包）、八斗子（台北市國家鐵道博物館）。
   **也就是說汙染的根源是 `blog-search` 這條路徑，不是 `youtube-search`**；關掉 YouTube 一點都沒有讓問題消失，只是讓人以為消失了。教訓與本節開頭那幾條同型：**「某個來源計數為 0」不等於「該症狀不會出現」——症狀要在畫面上實查，不能從欄位反推。**
   相關性過濾要補在 `blog-search`（最低門檻：標題／摘要須含景點名稱或別名），YouTube 那支重新啟用前也要補。
   ⚠️ **2026-08-04 的另一則更正仍然成立**：本檔更早之前寫「因未設 `YOUTUBE_DATA_API_KEY` 而不啟用」——**這個原因不成立**，root `.env` 裡該金鑰有值。線上為 0 的真正原因待查（可能是匯入時設了 `DISABLE_YOUTUBE_VALIDATOR=1`、配額耗盡或金鑰失效）。**結論不變：實測上限 5 類，對外不得寫「7 類交叉驗證」**；但決賽被問「為什麼沒有 YouTube」時，要答得出真實原因。
