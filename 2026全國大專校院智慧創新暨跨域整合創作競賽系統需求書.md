...
## 領航者 Navigator — 多人旅遊智能共識與即時韌性規劃系統
...
在多人旅遊的情境中，一群朋友、家人或同事要共同決定「去哪裡、住哪裡、怎麼排」時，往往面臨三個真實痛點：

1. **多人決策難收斂**：誰想去、誰不想去缺乏有效的表達與收斂機制，容易因「禮貌性點讚」讓沒人真正想去的景點被排進行程。
2. **行程缺乏應變邏輯**：遇到天氣、交通、景點臨時關閉等突發狀況時，沒有系統化的備案思路，只能臨場慌亂重排。
3. **網路景點資訊真假難辨**：部落格、社群、地圖上的資訊彼此衝突、品質不一，使用者難以判斷可信度。

觀察現有的旅遊規劃工具（TripAdvisor、Google Places、MindTrip 等），多聚焦於「景點推薦與資訊聚合」，鮮少有系統同時處理「群體共識收斂」與「行程進行中的即時韌性」。

本專案「領航者 Navigator」建立在一個**經多來源交叉驗證的可信景點知識庫**之上，強調三件事：以代幣投票達成**智能共識**、以錨點分級（L0–L3）與事件觸發達成**即時韌性**、以多來源信度與衝突透明化建立**資料可信度**。系統以涵蓋全台灣景點的廣度為核心差異化，用堪用且成本可控的 AI 技術（RAG 混合檢索、輕量 LLM），讓旅遊資訊「查得到、信得過、變得動」。
...
| Architect Agent（規劃代理） | 依群體偏好與投票結果，透過漏斗式檢索（SQL 結構化過濾 + 語意向量）產出草案行程。 |
| Contingency Handler Agent（應變處理代理） | 偵測天氣等事件，以期望值評估是否觸發應變，從知識庫撈出合格備案並產出 Swap／Switch 建議。 |
| Trip Room Module（行程房間模組） | 提供建立房間、多人加入、presence 管理。 |
| Exploration Module（景點探索模組） | 提供 Tinder 式滑卡探索、語意搜尋與資料可信度／衝突呈現。 |
| Voting Module（投票模組） | 提供代幣投票（VETO／MUST-GO／Like）與收斂結果。 |
| Itinerary Module（行程模組） | 提供草案行程檢視、拖拉編輯與地圖視覺化。 |
| Authentication Module（身分驗證模組） | 提供使用者註冊、登入與權限判定（Supabase Auth）。 |
| Ingestion Pipeline（入庫管線） | 將驗證後景點向量化並 upsert 至 `poi_catalog`，含 TDX 批次匯入。 |

> **[圖 1：系統架構圖 — 待補繪]**
> 內容：外部資料來源 →（POI Verifier Agent）→ `poi_catalog` 知識庫 →（Architect Agent／Contingency Handler Agent）→ 使用者介面；使用者回饋／投票／評價回流至資料來源層，形成資料閉環。可參照 `ARCHITECTURE.md` 與 `系統架構圖_0709.md` 之 Mermaid 圖。
...
**Trip Room Module（行程房間模組）**

| 編號 | 名稱 | 描述 |
|---|---|---|
| FFR1 | Create Trip Room（建立行程房間） | 使用者可建立一個行程房間，設定行程名稱、目的地區域與日期範圍，並取得可分享的加入連結／代碼。 |
| FFR2 | Join Trip Room（加入行程房間） | 使用者可透過連結或代碼加入既有行程房間，成為該行程的參與者，並看到房間內其他成員的即時狀態。 |

**Exploration Module（景點探索模組）**

| 編號 | 名稱 | 描述 |
|---|---|---|
| FFR3 | Swipe to Explore POIs（滑卡探索景點） | 系統以 Tinder 式卡片呈現候選景點，使用者以左右滑動（或按鈕）表達喜歡／略過，支援觸控手勢。 |
...
| 編號 | 名稱 | 描述 |
|---|---|---|
| FFR6 | Token Voting（代幣投票） | 每位參與者持固定代幣：1 張 VETO（否決）、2 張 MUST-GO（+5）、無限張 Like（+1），對候選景點投票。 |
| FFR7 | View Voting Results（檢視收斂結果） | 系統依 Σ(票 × 權重) 排序候選景點，VETO 者直接出局，呈現群體共識收斂後的景點排名。 |

**Itinerary Module（行程模組）**

| 編號 | 名稱 | 描述 |
|---|---|---|
| FFR8 | View Draft Itinerary（檢視草案行程） | 系統依投票結果自動產生的草案行程，以逐日、逐時段方式呈現給使用者檢視。 |
...
**Architect Agent Module（規劃代理模組）**

| 編號 | 名稱 | 描述 |
|---|---|---|
| BFR6 | Funnel Retrieval（漏斗式檢索） | 依區域、室內外、營業時段等結構化條件先過濾，再以語意向量補足 vibe，最後依 Level 重排，壓低 Token 成本。 |
| BFR7 | Draft Itinerary Generation（草案行程生成） | 依投票收斂結果與候選池，考量停留時間與景點間距，產生逐日草案行程骨架。 |
...
| EIR6 | User Authentication（Supabase Auth） | 使用者輸入帳號密碼或第三方登入以進入系統，系統判定其身分與權限。 |
...
| IIR3 | Voting Module 與 Database | 投票模組將各參與者的 VETO／MUST-GO／Like 寫入資料庫，並即時同步給房間內成員。 |
...
| IIR7 | Authentication Module 與 Trip Room Module | 使用者通過身分驗證後，方可建立或加入行程房間並使用房間內功能。 |
...
| 目標名稱 | 目標屬性 | 目標描述 |
|---|---|---|
| Reach Group Consensus（達成群體共識） | (R,A,F) | 讓多位參與者透過代幣投票，快速收斂出群體真正想去的景點，避免禮貌性點讚失真。 |
...
| Plan Trip Conveniently（便利規劃行程） | (R,A,F) | 讓發起人便利地建立房間、產生草案並編輯行程。 |
| Participate Conveniently（便利參與） | (R,A,F) | 讓參與者便利地加入房間、探索景點並投票。 |
| Manage Knowledge Base Conveniently（便利管理知識庫） | (S,A,F) | 讓管理員便利地批次驗證景點並維護知識庫。 |
| Identify Users（識別使用者） | (R,A,F) | 讓使用者可被識別並使用對應權限的系統功能。 |
...
> **[圖 2：目標導向使用案例圖 — 待補繪]**
> 內容：以上方目標套件（<<Goal>>）連接下方使用案例（橢圓）與角色（Actor：User、Trip Initiator、Participant、Administrator、Architect Agent、Contingency Handler Agent），標註 Achieve／Maintain／Optimize 及 <<include>>／<<extend>> 關係。
...
| Trip Initiator（行程發起人） | 建立行程房間、產生與編輯草案行程、確認應變建議的房主。 |
| Participant（參與者） | 受邀加入行程房間、滑卡探索並投票的成員。 |
| Administrator（系統管理員） | 維護 `poi_catalog` 知識庫，執行批次驗證與入庫的管理者。 |
| Architect Agent（規劃代理） | 依偏好與投票結果產生草案行程的系統角色。 |
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| UC-CU-001 滑卡探索景點 | V | V | V | | | |
| UC-CU-002 景點語意搜尋 | V | V | V | | | |
| UC-CU-003 檢視資料衝突 | V | V | V | | | |
| UC-CU-004 登入系統 | V | | | | | |
| UC-TI-001 建立行程房間 | | | V | | | |
| UC-TI-002 產生草案行程 | | | V | | V | |
| UC-TI-003 拖拉編輯行程 | | | V | | | |
| UC-PT-001 加入行程房間 | | V | | | | |
| UC-PT-002 代幣投票 | | V | V | | | |
...
| Use Case Name | Generate Draft Itinerary（產生草案行程） |
| Goal | Achieve "Plan Trip Conveniently" |
| Requirement | [FFR8: View Draft Itinerary]、[BFR6: Funnel Retrieval]、[BFR7: Draft Itinerary Generation] |
| Description | 系統依投票收斂結果，透過規劃代理自動產生逐日草案行程。 |
| Actor | Trip Initiator、Architect Agent |
| Priority | High |
| Pre-Conditions | 1. 房間已完成投票收斂　2. 知識庫有足夠候選景點 |
| Post-Conditions | 系統產生逐日、逐時段的草案行程供檢視 |
| Basic Flow | 1. 發起人點選「產生草案行程」<br>2. 規劃代理以漏斗式檢索取得候選池（結構化過濾 + 語意向量）<br>3. 規劃代理依投票排名、停留時間與景點間距安排時段<br>4. 系統產生逐日草案行程並顯示於行程頁 |
| Alternative Flows | 3.1 若候選景點不足以填滿日程，系統以 L3 水位調節景點補足 buffer |
| Exceptional Flows | 2.1 若投票尚未收斂，系統提示需先完成投票 |
| Use Use Case | [UC-AG-001: 混合檢索] |
| Extend Use Case | [UC-TI-001: 建立行程房間] |
| Artifacts | 草案行程 |
