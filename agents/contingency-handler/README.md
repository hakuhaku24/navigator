...
| 檢查項   | 資料來源             | 篩掉條件                | 狀態 |
| -------- | -------------------- | ----------------------- | ---- |
| 人潮     | **未接**             | > 80%（extremely_busy） | ⚠️ 規則存在但不會觸發 |
| 營業時間 | 距離關閉時間         | < 5 分鐘                | ✅ |
| 資訊時效 | 上次更新距今         | > 30 天                 | ✅ |
...
> ⚠️ **關於「人潮」這一條**
>
> 本表先前記載此欄資料來自「Google Maps 實時人流」。**該資料沒有任何官方 API**——
> Google 的 Popular Times / Live busyness 只出現在 Maps 與搜尋的使用者介面，
> 未開放於 Places API，僅能靠爬取 Maps 網頁取得（違反服務條款且極易失效）。
> 因此本專案**沒有、也不打算接**這個資料來源。
>
> `strict-checker.ts` 的 `current_crowd_level === 'extremely_busy'` 規則**保留**，
> 因為「不知道就不淘汰」是正確行為；但 `current_crowd_level` 目前全 repo 無 producer，
> **這條規則實際上從未淘汰過任何候選景點**。
>
> 對外文件與簡報不得宣稱本系統會避開人潮擁擠的景點。
