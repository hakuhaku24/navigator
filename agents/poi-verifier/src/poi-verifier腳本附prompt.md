...
const DEFAULT_WEIGHTS: MultiCriteriaWeights = {
  rating: 0.25, // 官方評分（0–5）
  review_count: 0.15, // 評論數（多簽），log scale
  distance: 0.2, // 距離（近優先），inverse scale
  opening_hours_margin: 0.15, // 營業時間餘裕（分鐘）
  cost_within_budget: 0.1, // 費用符合 true/false
  weather_compatibility: 0.2, // 晴/雨 compatibility
  crowd_capacity: 0.1, // 人潮容納度
  energy_consumption: 0.1, // 體力消耗（低優先）
  group_preference_match: 0.15, // 族群偏好
  source_credibility_boost: 0.05, // 官方來源 +5%
  recency_bonus: 0.05, // 近期更新 +5%
};
...
function adjustWeightsForScenario(
  ...
  switch (scenario) {
    case "heavy_rain":
      return { ...DEFAULT_WEIGHTS, weather_compatibility: 0.4, distance: 0.15 };
    case "closure":
      return { ...DEFAULT_WEIGHTS, opening_hours_margin: 0.3, rating: 0.15 };
    case "fatigue":
      return { ...DEFAULT_WEIGHTS, energy_consumption: 0.25, distance: 0.15 };
  }
}
...
function scoreCandidate(
  ...
  score +=
    (poi.primary_source_credibility || 0) *
    weights.source_credibility_boost *
    100;
  score +=
    (1 - Math.min(poi.days_since_update / 180, 1)) *
...
function generateBackupLogic(
  ...
  if (level === 0) {
    return null; // L0 不生成 backup_logic
  }
  ...
  const qualified = candidatePool.filter((candidate) => {
    ...
    if (candidate.crowd_level === "extremely_busy") {
  ...
  // 多準則排序
  const weights = adjustWeightsForScenario(context.scenario);
