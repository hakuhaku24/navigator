...
const DEFAULT_WEIGHTS: MultiCriteriaWeights = {
  rating: 0.20,                 // 官方評分（0–5）
  review_count: 0.10,           // 評論數，log scale
  distance: 0.15,               // 距離（近優先），inverse scale
  opening_hours_margin: 0.10,   // 營業時間餘裕（分鐘）
  cost_within_budget: 0.05,     // 費用符合 true/false
  weather_compatibility: 0.15,  // 晴/雨 compatibility
  crowd_capacity: 0.05,         // 人潮容納度
  energy_consumption: 0.05,     // 體力消耗（低優先）
  group_preference_match: 0.10, // 族群偏好
  source_credibility_boost: 0.025, // 官方來源加分
  recency_bonus: 0.025,         // 近期更新加分
};
