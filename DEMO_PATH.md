...
> ⚠️ **2026-07-16 拍板：本文件已過時。** Step 1–3、6、8 描述的建房間／Tinder swipe／Token 投票收斂流程已整條移出範圍（定位收斂為單人使用情境＋可信景點資料庫＋即時韌性應變）。程式碼保留未刪，但不再照此腳本 demo。**Step 4 提到的「Architect Agent」一詞也已停用**——行程草案不是 LLM 產出的，是使用者選點後純規則排序（`src/lib/draft-itinerary.ts`），沒有 LLM 呼叫，也不在目前主線待辦內，見 `系統架構圖_競賽版.md`。完整決策見 `0716_減法決策與不做清單.md`；新版 demo 主線見 `系統架構圖_競賽版.md`。本文件保留作歷史參考，勿依此規劃工時。
